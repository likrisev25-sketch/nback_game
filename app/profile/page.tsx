...
export default async function ProfilePage() {
  ...
  // Фильтруем ботов
  const userGames = gameHistory.filter(g => !g.isBot);

  // Общая статистика
  const totalGames = userGames.length;
  const totalCorrect = userGames.reduce((sum, g) => sum + (g.correctAnswers || 0), 0);
  const totalErrors = userGames.reduce((sum, g) => sum + (g.errors || 0), 0);
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6 mb-8">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">
                Регистрация: {new Date(session.user.createdAt || Date.now()).toLocaleDateString('ru-RU')}
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 text-center">
              ...
              <div className="text-xs text-gray-500 dark:text-gray-400">Правильных ответов</div>
            </div>
            <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 text-center">
              <div className="text-2xl font-bold text-red-600">{totalErrors}</div>
              <div className="text-xs text-gray-500 dark:text-gray-400">Ошибок</div>
            </div>
            <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 text-center">
              ...
              <table className="w-full">
                ...
                <tbody>
                  {userGames.map((game, index) => {
                    const gameAccuracy = game.correctAnswers + game.errors > 0
                      ? Math.round((game.correctAnswers / (game.correctAnswers + game.errors)) * 100)
                      : 0;
